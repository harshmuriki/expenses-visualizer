...
import {
  uploadTransactionsBatch,
  getUserFiles,
  getUserMonths,
} from "@/lib/storageAdapter";

// Feature Showcase Component
const FeatureShowcase: React.FC<{ isLightTheme: boolean }> = ({
  isLightTheme,
}) => {
  ...
  const features = [
    {
      ...
      description:
        "Automatically categorize your transactions using advanced AI. Save hours of manual work.",
      icon: "🤖",
    ...
    },
    {
      ...
      description:
        "Interactive treemaps, charts, and graphs. Understand your spending at a glance.",
      icon: "📊",
    ...
    },
    {
      ...
      description:
        "Track spending patterns across months. See where your money goes over time.",
      icon: "📈",
    ...
    },
    {
      ...
      description:
        "Get personalized recommendations and anomaly detection powered by AI.",
      icon: "💡",
    ...
    },
    {
      ...
      description:
        "Upload CSV files from any bank for automatic transaction processing.",
      icon: "🏦",
  ...
  return (
    <div
      className="group relative overflow-hidden rounded-3xl p-8 hover:scale-[1.01] transition-all duration-300 border-2 shadow-xl min-h-[400px] flex flex-col"
      style={{
    ...
    >
      ...
      <div className="relative z-10 flex-1 flex flex-col">
        ...
        <div
          key={currentFeature}
          className="flex-1 flex flex-col justify-center space-y-6 fade-in-up"
        >
          {/* Feature Icon */}
...
const HomePage: React.FC = () => {
  ...
  const [activeStorageMode, setActiveStorageMode] = React.useState<StorageMode>(
    getStorageMode()
  );
  const [localDirectoryHandle, setLocalDirectoryHandle] =
    React.useState<any>(null);
  const [localFiles, setLocalFiles] = React.useState<string[]>([]);
  const [firebaseMonths, setFirebaseMonths] = React.useState<string[]>([]);
  const [isLoadingLocalFiles, setIsLoadingLocalFiles] = React.useState(false);
  const [isLoadingFirebaseFiles, setIsLoadingFirebaseFiles] =
  ...
  // Refresh storage mode badge on mount (and when user triggers manually)
  React.useEffect(() => {
    setActiveStorageMode(getStorageMode());
  }, []);

  const ensureDirectoryPermission = async (handle: any) => {
  ...
  const listLocalJsonFiles = async (directoryHandle: any) => {
    ...
    try {
      ...
      alert("Failed to list local files.");
    } finally {
      setIsLoadingLocalFiles(false);
    }
  };

  const loadLocalJsonFile = async (fileName: string) => {
    if (!session?.user?.email) {
      alert("You need to be signed in to load data.");
      return;
    }
    let directoryHandle = localDirectoryHandle;
    if (!directoryHandle) {
      directoryHandle = await connectLocalFolder();
    }
    if (!directoryHandle) return;
    ...
    try {
      ...
      if (!(await ensureDirectoryPermission(fileHandle))) {
      ...
      const userEmail = parsed?.userEmail || session.user.email || "";
      ...
      alert(`Loaded ${fileName}. Redirecting to chart...`);
  ...
  const refreshLocalFiles = async () => {
    ...
    if (directoryHandle) {
      await listLocalJsonFiles(directoryHandle);
    }
  };

  const refreshFirebaseFiles = React.useCallback(async () => {
    if (!session?.user?.email) return;
    setIsLoadingFirebaseFiles(true);
    try {
      const months = await getUserMonths(session.user.email);
      setFirebaseMonths(months || []);
    } catch (err) {
      console.error("Error fetching firebase months", err);
    } finally {
      setIsLoadingFirebaseFiles(false);
    }
  }, [session?.user?.email]);

  React.useEffect(() => {
    refreshFirebaseFiles();
  }, [refreshFirebaseFiles]);

  React.useEffect(() => {
    if (activeStorageMode === "local") {
      if (localDirectoryHandle) {
        listLocalJsonFiles(localDirectoryHandle);
      }
    } else {
      refreshFirebaseFiles();
    }
  }, [activeStorageMode, localDirectoryHandle, refreshFirebaseFiles]);
    ...
    <div className="flex flex-col min-h-screen bg-background-primary relative overflow-hidden">
      ...
      {isNavigatingToTrends && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/30 backdrop-blur-sm">
          <div
            className="flex flex-col items-center space-y-4 bg-background-card p-8 rounded-2xl border-2 shadow-2xl"
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        <div
          className="absolute -bottom-40 -left-40 w-96 h-96 bg-gradient-to-br from-accent-400/30 to-secondary-400/30 rounded-full blur-3xl animate-pulse"
      ...
      <header className="relative z-10 py-8 md:py-12 text-center px-4">
        ...
        <div className="absolute top-4 right-4 md:top-8 md:right-8 z-20">
          <div
            className="flex items-center gap-3 rounded-full border-2 shadow-xl transition-all hover:scale-105"
            style={{
              backgroundColor: "var(--color-background-card)",
              borderColor: "var(--color-border-secondary)",
              padding: "8px 16px",
        ...
        <div className="mb-6 flex justify-center">
          <div
            className="relative inline-flex items-center gap-3 rounded-full px-6 py-3 shadow-xl border-2 transition-all duration-300 hover:scale-105"
            style={{
              backgroundColor:
        ...
        <div className="relative mb-8">
          ...
          <h1 className="relative text-6xl md:text-8xl font-black mb-4 leading-[0.9] tracking-tight">
            ...
            <span className="block gradient-text-animated">Intelligence</span>
          </h1>
        ...
        <div className="max-w-3xl mx-auto mb-12">
          ...
          <p
            ...
            style={{ color: "var(--color-text-secondary)" }}
          >
            Upload, analyze, visualize. Your spending patterns decoded in
            seconds.
          </p>
        </div>
        ...
        <div className="flex flex-wrap justify-center gap-4 md:gap-6 mt-8">
          <div
            className="group relative rounded-2xl px-6 py-5 min-w-[150px] hover:scale-105 transition-all duration-300 border-2 shadow-lg"
            style={{
              backgroundColor: "var(--color-background-card)",
          ...
          >
            <div className="relative">
              <div
                className="text-4xl md:text-5xl font-black mb-3 group-hover:scale-110 transition-transform duration-300"
                style={{ color: "var(--color-primary-500)" }}
              >
                ∞
              </div>
      ...
      <nav className="relative z-20 flex justify-center mb-6 px-4">
        <div
          className="flex items-center gap-3 rounded-2xl p-3 border-2 shadow-lg"
          style={{
            backgroundColor: "var(--color-background-card)",
            borderColor: "var(--color-border-secondary)",
          }}
      ...
      <main className="relative z-10 flex-grow flex items-center justify-center px-4 pb-12">
        <div className="w-full max-w-7xl space-y-8">
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
            {/* Master Storage Card */}
            <div
              className="group relative overflow-hidden rounded-3xl p-8 hover:scale-[1.01] transition-all duration-300 border-2 shadow-xl"
              style={{
                backgroundColor: "var(--color-background-card)",
                borderColor: "var(--color-border-secondary)",
              }}
            >
              ...
              <div className="relative z-10 space-y-6">
                ...
                <div
                  className="p-4 rounded-xl border-2 bg-background-secondary"
                  style={{ borderColor: "var(--color-border-secondary)" }}
                >
                  <StorageModeToggle
                    showLabel={true}
                    size="md"
                    onModeChange={(mode) => setActiveStorageMode(mode)}
                  />
                </div>

                {/* Local / Firebase Cards - switched by activeStorageMode */}
                {activeStorageMode === "local" ? (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2">
                        <button
                          ...
                          style={{
                            backgroundColor:
                              "var(--color-background-card, #111827)",
                            borderColor: "var(--color-border-secondary)",
                    ...
                    <button
                      onClick={async () => {
                        ...
                        if (handle) {
                          await listLocalJsonFiles(handle);
                        }
                      }}
                      className="w-full px-4 py-3 rounded-lg border text-sm font-semibold transition hover:scale-[1.01]"
                      style={{
                        backgroundColor: "var(--color-background-card)",
                        borderColor: "var(--color-border-secondary)",
                    ...
                    >
                      {localDirectoryHandle?.name
                        ? `Connected to: ${localDirectoryHandle.name}`
                        : "Connect folder"}
                    </button>
                    <div
                      className="rounded-lg border p-3 h-40 overflow-y-auto"
                      style={{ borderColor: "var(--color-border-secondary)" }}
                    >
                      <p
                      ...
                      {isLoadingLocalFiles ? (
                        ...
                        <div className="space-y-2">
                          {localFiles.map((file) => (
                            <button
                              key={file}
                              onClick={() => loadLocalJsonFile(file)}
                              className="w-full text-left px-3 py-2 rounded-lg border transition hover:scale-[1.01]"
                              style={{
                                backgroundColor: "var(--color-background-card)",
                                borderColor: "var(--color-border-secondary)",
                                color: "var(--color-text-primary)",
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <FiCloud className="text-primary-500" />
                        <h3
                          className="font-semibold"
                          style={{ color: "var(--color-text-primary)" }}
                        >
                    ...
                    >
                      ...
                      {isLoadingFirebaseFiles ? (
                        ...
                        <div className="space-y-2">
                          {firebaseMonths.map((month) => (
                            <button
                              key={month}
                              onClick={() =>
                                router.push(
                                  `/chart?month=${encodeURIComponent(month)}`
                                )
                              }
                              className="w-full text-left px-3 py-2 rounded-lg border transition hover:scale-[1.01]"
                              style={{
          ...
          <div className="relative z-10 flex justify-center">
            <div
              className="w-full max-w-5xl rounded-3xl p-8 border-2 shadow-xl"
              style={{
                backgroundColor: "var(--color-background-card)",
                borderColor: "var(--color-border-secondary)",
              }}
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                ...
                <div
                  className="group relative flex flex-col items-start p-5 rounded-xl border-2 transition-all duration-300 hover:shadow-lg hover:scale-[1.02]"
                  style={{
                    backgroundColor: "var(--color-background-secondary)",
                    borderColor: "var(--color-secondary-500)",
                  }}
                >
                  ...
                  <p
                    ...
                    style={{ color: "var(--color-text-secondary)" }}
                  >
                    Multi-month analysis with AI-powered insights and
                    predictions
                  </p>
                </div>
                ...
                >
                  ...
                  <p
                    ...
                    style={{ color: "var(--color-text-secondary)" }}
                  >
                    Upload CSV files and let AI automatically categorize your
                    transactions
                  </p>
                </div>
                ...
                >
                  ...
                  <p
                    ...
                    style={{ color: "var(--color-text-secondary)" }}
                  >
                    Get intelligent insights and personalized spending
                    recommendations
                  </p>
                </div>
              </div>
